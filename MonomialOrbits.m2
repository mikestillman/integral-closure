...
orbitRepresentatives(Ring, Ideal, VisibleList) := List => o -> (R, I, degs) -> (
    ...
    if I isMonomialIdeal I then result := I else(
    <<"warning: arg 1 may not be a monomial ideal"<<endl;
...
orbitRepresentatives(Ring, Ideal, Ideal, ZZ) := List => o -> (R, I, startmons, numelts) -> (
    ...
    if not isMonomialIdeal I then <<"warning: arg 1 may not be generated by monomials"<<endl;
    J := monomialIdeal I;

    if not isMonomialIdeal startmons then(
    <<"warning: arg 2 may not be generated by monomials"<<endl;
    );

    G := permutations R;
    start := compress ((gens startmons) % J);

    if numelts < 0 then(
	    L := flatten entries start;
    	    sL := subsets(L, #L+numelts)/monomialIdeal;
    	    return normalForms(apply(sL, ell -> ell + J), G)
    ) else

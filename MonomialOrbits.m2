...
doc ///
    ...
    Description
        Text
            on the variables of a polynomial ring. The set of
            monomials is specified by generator degrees in @TO
            orbitRepresentatives @ or Hilbert function in @TO
            hilbertRepresentatives@.  If the option MonomialType =>
...
doc ///
    ...
    Description
        Text
            The script creates the ring transformations defined by
            permuations of variables.  If no Group is specified, uses
            the full symmetric group, but it is also possible to
            specify an arbitrary list of permutations. It then adds
            one generator at a time starting with the lowest degrees
            specified; after each addition it chooses representatives
            under the group action.
     
            If the option @TO MonomialType@ => "SquareFree" is set
...
doc ///
    ...
    Description
        Text
            ...
            permuations of variables.  If no Group is specified, uses
            the full symmetric group, but it is also possible to
            specify an arbitrary list of permutations. Starting with
            orbit representatives of monomial ideals generated by s_0
            quadrics, it successively adds to each as many forms of
            degree d in (3..1+length s) as necessary to achieve the
            desired Hilbert function.  After each addition it chooses
            ...
            VisibleList, which could be either a list or a sequence.
        Example
            S = ZZ/101[a..d]
